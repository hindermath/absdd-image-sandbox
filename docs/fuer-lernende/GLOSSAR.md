...
| **Agenten-Parität** | Gleichwertige Behandlung der vier Required-Agenten (Codex, Claude Code, Gemini CLI, GitHub Copilot CLI) und des zusätzlichen OpenCode-Werkzeugs — Regeln gelten für alle. / Equal treatment of the four required agents and the additional OpenCode tool — rules apply to all. |
