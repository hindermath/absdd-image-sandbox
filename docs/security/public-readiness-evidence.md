...
| Required-Agenten-Status / Required agent status | `Open` | `Dockerfile`, `compose.yml`, `scripts/smoke-test-toolchains.sh`, `docs/security/ai-tools-inventory.md` | Codex, Claude Code, Antigravity CLI und GitHub Copilot CLI sind gepinnt und technisch pruefbar; Konto-, Anbieter-, Rechts- und Modellfreigaben bleiben menschliche Entscheidungen. | Owner / KIB | Freigaben je Anbieter und Betriebsmodell dokumentieren. | Anbieter-, Konto- oder Modellfreigabe |
...
| PowerShell-Skriptgrundlage / PowerShell scripting foundation | active-evidence | `Dockerfile`, `compose.yml`, `.env.example`, `scripts/agent-prompt.*`, `scripts/smoke-test-toolchains.sh`, `README.md` | Das MCR-.NET-SDK-Basisimage liefert das im Build auf `7.6.1` gepruefte PowerShell 7; `/powershell-projects` ist ein expliziter Mount und beide Dispatchervarianten werden auf Paritaet geprueft. | Maintainer | Basisimage, Versions-ARG, Mount, Dispatcher und Smoke-Test synchron halten. | PowerShell-, Basisimage-, Mount- oder Dispatcher-Aenderung |
...
| PowerShell scripting foundation | active-evidence | `Dockerfile`, `compose.yml`, `.env.example`, `scripts/agent-prompt.*`, `scripts/smoke-test-toolchains.sh`, `README.md` | The MCR .NET SDK base image supplies PowerShell 7, checked as `7.6.1` during the build; `/powershell-projects` is explicit and both dispatcher variants have parity tests | Maintainer | Keep the base image, version ARG, mount, dispatchers, and smoke test aligned | PowerShell, base image, mount, or dispatcher change |
