...
| Agenten-CLIs | Exakt gepinnte npm-Pakete fuer Codex, Claude Code, Antigravity CLI und GitHub Copilot CLI | Beibehalten; Renovate-Vorschlaege nur nach Build, Versionscheck, Smoke-Test und SBOM mergen | erledigt in Vier-Agenten-Lernumgebung |
...
| Agent CLIs | Exactly pinned npm packages for Codex, Claude Code, Antigravity CLI, and GitHub Copilot CLI | Keep; merge Renovate proposals only after build, version checks, smoke test, and SBOM | done in four-agent learner environment |
