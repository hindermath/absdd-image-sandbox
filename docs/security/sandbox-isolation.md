...
| Supply-Chain-Kontrolle | `Dockerfile`: gepinntes Basisimage per `sha256`-Digest, gepinnte Tool-ARGs fuer OpenCode, Codex, Spec Kit, Go und Rust; `docs/security/ai-tools-inventory.md`; `scripts/build-and-sbom.*` | Das Basisimage ist reproduzierbar per Digest referenziert. Fuer das finale Image kann eine CycloneDX-SBOM erzeugt werden. P3-1 bleibt offen, weil einzelne Installerpfade noch durch verifiziertere Verfahren ersetzt werden sollen. |
...
| Supply-chain control | `Dockerfile`: base image pinned by `sha256` digest, pinned tool ARGs for OpenCode, Codex, Spec Kit, Go, and Rust; `docs/security/ai-tools-inventory.md`; `scripts/build-and-sbom.*` | The base image is referenced reproducibly by digest. A CycloneDX SBOM can be generated for the final image. P3-1 remains open because some installer paths still need to be replaced by more verified methods. |
...
- P3-1: Further harden installer paths and replace curl-pipe-bash patterns.
