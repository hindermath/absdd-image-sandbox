...
| Codex-Schreibgrenzen | `codex/config.toml`: `sandbox_mode = "workspace-write"`, `[sandbox_workspace_write].writable_roots`; `codex/requirements.toml`: `allowed_sandbox_modes`, `permissions.filesystem.deny_read` | Codex darf nur in definierten Arbeitsverzeichnissen schreiben, einschliesslich `/ade-dev-sandbox` fuer dieses Setup-Repository, `/secure-case-tracker-projects`, `/secure-service-harvester-projects` und `/secure-order-desk-projects` fuer die Lernreihen-Projektfamilien sowie `/home/adedev/home-baseline-tmp` fuer den optionalen Home-Baseline-Checkout. Sensitive Pfade wie `.ssh`, `.gnupg`, `.codex`, OpenCode-Daten und Secret-Dateien sind als Leseverbote dokumentiert. |
...
| Supply-Chain-Kontrolle | `Dockerfile`: MCR-.NET-SDK-Basisimage per `sha256`-Digest, signierte NodeSource-Apt-Quelle, verifizierte uv-, Swift- und Syft-Release-Artefakte, gepinnte Tool-ARGs fuer OpenCode, die vier Required-Agenten, Spec Kit und die Sprachtoolchains; `docs/security/ai-tools-inventory.md`; `docs/security/supply-chain-todo.md`; `scripts/build-and-sbom.*` | Das Basisimage ist reproduzierbar per Digest referenziert. Release-Artefakte werden vor Installation kryptografisch geprueft. Alle Agenten-CLIs sind exakt gepinnt und Renovate-verwaltet. Fuer das finale Image kann eine CycloneDX-SBOM erzeugt werden. |
...
| Codex write boundaries | `codex/config.toml`: `sandbox_mode = "workspace-write"`, `[sandbox_workspace_write].writable_roots`; `codex/requirements.toml`: `allowed_sandbox_modes`, `permissions.filesystem.deny_read` | Codex may write only to defined work directories, including `/ade-dev-sandbox` for this setup repository, `/secure-case-tracker-projects`, `/secure-service-harvester-projects`, and `/secure-order-desk-projects` for the learning-series project families, and `/home/adedev/home-baseline-tmp` for the optional home-baseline checkout. Sensitive paths such as `.ssh`, `.gnupg`, `.codex`, OpenCode data, and secret files are documented as read-denied paths. |
...
| Supply-chain control | `Dockerfile`: MCR .NET SDK base image pinned by `sha256` digest, signed NodeSource Apt source, verified uv, Swift, and Syft release artifacts, and pinned tool ARGs for OpenCode, the four required agents, Spec Kit, and language toolchains; `docs/security/ai-tools-inventory.md`; `docs/security/supply-chain-todo.md`; `scripts/build-and-sbom.*` | The base image is reproducibly referenced by digest. Release artifacts are cryptographically verified before installation. Every agent CLI is exactly pinned and Renovate-managed. A CycloneDX SBOM can be generated for the final image. |
