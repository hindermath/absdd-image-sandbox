...
| Supply-chain control | `Dockerfile`: MCR .NET SDK base image pinned by `sha256` digest, signed NodeSource Apt source, uv release artifact with SHA256 verification, Swift release artifact with PGP signature verification, pinned tool ARGs for OpenCode, Codex, Spec Kit, Go, Rust, and Swift; `docs/security/ai-tools-inventory.md`; `docs/security/supply-chain-todo.md`; `scripts/build-and-sbom.*` | The base image is referenced reproducibly by digest. Node.js is installed through a signed Apt source instead of an executed setup script. uv and uvx are installed from a fixed release artifact after SHA256 verification. Swift is downloaded from a fixed `download.swift.org` release artifact and verified by PGP signature before extraction. A CycloneDX SBOM can be generated for the final image. |
