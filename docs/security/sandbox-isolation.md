...
| Dateisystem-Grenzen | `compose.yml`: Service `ade`, Abschnitt `volumes`; optional `compose.home-baseline.yml`; `docs/security/sandbox-freigabe.md`: "Genehmigte Mount-Liste" | Host-Dateien sind nur ueber explizit konfigurierte Bind-Mounts sichtbar: `/ade-dev-sandbox`, `/workspace`, `/rider-projects`, `/java-projects`, `/go-projects`, `/rust-projects`, `/python-projects`, `/powershell-projects`, `/swift-projects`, `/secure-case-tracker-projects`, `/secure-service-harvester-projects`, `/secure-order-desk-projects`, `/audit` und optional `/opt/home-baseline`. Die eingebaute Level-0-Quelle liegt root-eigen und ohne Schreibbits unter `/opt/home-baseline`; `~/home-baseline-source` ist der kanonische Symlink und `~/home-baseline-tmp` der veraltete Kompatibilitaetslink. Der optionale persoenliche Mount ueberdeckt dieses Ziel. Die .NET-Konfiguration `./dotnet/ContainerBuild.props` ist mit `:ro` read-only gemountet. |
...
| Codex-Schreibgrenzen | `codex/config.toml`: `sandbox_mode = "workspace-write"`; `codex/requirements.toml`: `allowed_sandbox_modes`, `permissions.filesystem.deny_read`; `scripts/codex-bwrap-wrapper.sh` | Codex schreibt ausschliesslich im image-eigenen Transaktionspfad `/home/adedev/codex-workspace`. Dessen `.git`-, `.agents`- und `.codex`-Platzhalter sind root-eigen und read-only; sensitive Benutzer- und Providerpfade bleiben unlesbar. Host-Projektmounts dienen agentenlosen Installations- und Prueflaeufen sowie kontrolliertem Import und Export. |
...
| Netzwerkstatus | `docs/security/network-decision.md`: freier Egress als Open dokumentiert; `codex/config.toml`: `[sandbox_workspace_write].network_access = false` | Die Containerumgebung nutzt den normalen Compose-Netzwerkpfad. Innerhalb von Codex ist Netzwerkzugriff im Workspace-Write-Sandboxmodus deaktiviert. Die Egress-Grenze ist ein offener Plattformpunkt, keine agentische Risikoakzeptanz. |
...
| Supply-Chain-Kontrolle | `Dockerfile`; `home-baseline.lock.json`; `scripts/sync-home-baseline-runtime.sh`; MCR-.NET-SDK-Basisimage per `sha256`-Digest, verifizierte Release-Artefakte und gepinnte Tool-ARGs; `scripts/build-and-sbom.*` | Das Basisimage ist per Digest referenziert. Die eingebettete MIT-lizenzierte Level-0-Quelle wird als Shallow-Git-Checkout aus einem Release geladen; Build und Lock-Validator erzwingen Tag und exakten Commit. Keine Baseline-Skripte laufen automatisch. Nur der ausdrueckliche Runtime-Wrapper darf manifestdefinierte Runtime-Inhalte konfliktgeschuetzt nach `/home/adedev` schreiben. Fuer das finale Image kann eine CycloneDX-SBOM erzeugt werden. |
...
| Filesystem boundaries | `compose.yml`: service `ade`, `volumes` section; optional `compose.home-baseline.yml`; `docs/security/sandbox-freigabe.md`: approved mount list | Host files are visible only through explicitly configured bind mounts, including optional `/opt/home-baseline`. The embedded Level-0 source is root-owned without write bits at `/opt/home-baseline`; `~/home-baseline-source` is canonical and `~/home-baseline-tmp` is the deprecated compatibility link. The optional personal mount covers this target. The .NET configuration remains mounted read-only with `:ro`. |
...
| Codex write boundaries | `codex/config.toml`: `sandbox_mode = "workspace-write"`; `codex/requirements.toml`: `allowed_sandbox_modes`, `permissions.filesystem.deny_read`; `scripts/codex-bwrap-wrapper.sh` | Codex writes only inside the image-owned transaction path `/home/adedev/codex-workspace`. Its `.git`, `.agents`, and `.codex` placeholders are root-owned and read-only; sensitive user and provider paths remain unreadable. Host project mounts support agentless installation and tests plus controlled import and export. |
...
| Network status | `docs/security/network-decision.md`: unrestricted egress documented as Open; `codex/config.toml`: `[sandbox_workspace_write].network_access = false` | The container uses the normal Compose network path. Codex workspace-write network access is disabled. The egress boundary remains an open platform item, not agent risk acceptance. |
...
| Supply-chain control | `Dockerfile`; `home-baseline.lock.json`; `scripts/sync-home-baseline-runtime.sh`; MCR .NET SDK base image pinned by digest, verified release artifacts, and pinned tool arguments; `scripts/build-and-sbom.*` | The embedded MIT-licensed Level-0 source is a shallow Git checkout from a release; the build and lock validator enforce its tag and exact commit. No baseline script runs automatically. Only the explicit runtime wrapper may write manifest-defined Runtime content to `/home/adedev`, with conflict protection. A CycloneDX SBOM can be generated for the final image. |
